...
class DependencyExtractorVisitor extends RecursiveAstVisitor<void> {
  ...
  void _handleElement(Element element) {
    ...
    if (element is ConstructorElement) {
      ...
      if (enclosing is ClassElement ||
        ...
        if (enclName != null && enclName != enclosingClass?.namePart.typeName.lexeme) {
          target = enclosing;
          name = enclName;
        } else {
          target = element;
    ...
    try { filePath = (target as dynamic)?.library?.source?.fullName; } catch (_) {}
    try { filePath ??= (element as dynamic)?.source?.fullName; } catch (_) {}
    ...
    if (!isProjectLocal(filePath)) {
      final uri = target?.library?.identifier ?? element.library?.identifier ?? '';
  ...
  void visitNamedType(NamedType node) {
    ...
    Element? element;
    try { element = (node as dynamic).element as Element?; } catch (_) {}
    if (element == null) {
      try { element = (node as dynamic).type?.element as Element?; } catch (_) {}
    }
    if (element == null) {
      try { element = (node.type as dynamic)?.element as Element?; } catch (_) {}
    }
    if (element != null) {
